...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ---- Menú móvil ----
     Todo el cierre pasa por closeMenu(). Antes la cruz del panel llevaba su
     propio onclick en el HTML que solo quitaba la clase 'open': dejaba el
     <body> con overflow:hidden y el ícono en cruz, y la página se quedaba
     trabada. Ese onclick se retiró; la cruz es ahora #menuCerrar. */
  ...
  /* Ancho a partir del cual el menú lateral deja de usarse. Debe coincidir con
     el prefijo de Tailwind que oculta el botón y el panel en el HTML. */
  var ANCHO_ESCRITORIO = 1280;
  ...
  function closeMenu() {
    ...
    /* Se limpia todo lo que pudiera estar bloqueando el desplazamiento, no solo
       lo que abrimos nosotros: al pulsar "atrás", Android Chrome restaura la
       página desde memoria con estos estilos ya puestos. */
  ...
  if (menuToggle && mobileMenu) {
    ...
    /* La cruz del panel. */
    if (menuCerrar) menuCerrar.addEventListener('click', closeMenu);

    /* Cualquier enlace del panel, incluidos el botón de asesoría y las tres
       redes sociales: al navegar, el menú nunca debe quedarse abierto. */
    mobileMenu.querySelectorAll('a').forEach(function (enlace) {
      enlace.addEventListener('click', closeMenu);
    });

    /* No hay fondo oscuro que escuchar, así que se vigila el documento: un
       toque fuera del panel y fuera del botón del encabezado cierra el menú. */
    ...
    /* Al ensanchar hasta escritorio el panel desaparece por CSS; si no se
       cerrara también por dentro, el <body> quedaría bloqueado sin menú a la
       vista y sin forma de desbloquearlo. */
    window.addEventListener('resize', function () {
      if (window.innerWidth >= ANCHO_ESCRITORIO && menuAbierto()) closeMenu();
    });

    /* Segunda vía, independiente de 'resize': matchMedia avisa al cruzar el
       punto de quiebre aunque el navegador no emita ese evento —cambio de
       orientación, zoom o emulación de dispositivo—. Sin esto, en un navegador
       que no lo emitiera el menú quedaría abierto y el <body> bloqueado. */
    var mqEscritorio = window.matchMedia('(min-width: ' + ANCHO_ESCRITORIO + 'px)');
    var alCruzarElPuntoDeQuiebre = function (e) { if (e.matches) closeMenu(); };
    if (mqEscritorio.addEventListener) { mqEscritorio.addEventListener('change', alCruzarElPuntoDeQuiebre); }
    else if (mqEscritorio.addListener) { mqEscritorio.addListener(alCruzarElPuntoDeQuiebre); }
  }
  ...
  /* Redes de seguridad: al cargar la página y al regresar desde la memoria del
     navegador, que es donde el defecto reaparecía en Android. */
  closeMenu();
...
/* Propuesta 2026-09-17: hilo de lectura (la línea que avanza con el scroll). Se crea aquí para no tocar las 17 páginas. */
(function(){ var h=document.createElement('div'); h.className='hilo-lectura'; h.setAttribute('aria-hidden','true'); document.addEventListener('DOMContentLoaded',function(){document.body.prepend(h);}); })();
